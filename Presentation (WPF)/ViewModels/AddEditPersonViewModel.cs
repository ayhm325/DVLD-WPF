...
public partial class AddEditPersonViewModel : ObservableObject
{
    ...
    [ObservableProperty] private CountryResponse? _selectedCountry;
    ...
    [ObservableProperty] private string _nationalNo = string.Empty;
    ...
    public ObservableCollection<CountryResponse> Countries { get; } = [];
    ...
    partial void OnSelectedCountryChanged(CountryResponse? value) =>
    ...
    public async Task InitializeAsync(int? personId)
    {
        try
        {
            var countriesResult = await _countriesApiClient.GetAllAsync();

            if (countriesResult.IsFailure)
            {
                _notifications.ShowFailure(
                    countriesResult,
                    "Load Countries Failed");
                return;
            }
            ...
            {
                var personResult =
                    await _peopleApiClient.GetByIdAsync(personId.Value);
    ...
    private void LoadData(PersonResponse person)
    {
        ...
        SelectedCountry =
            Countries.FirstOrDefault(
                c => c.CountryId == person.NationalityCountryID)
            ?? Countries.FirstOrDefault();
    ...
    private async Task SavePersonAsync()
    {
        ...
        if (Mode == OperationMode.Edit)
        {
            var request = new UpdatePersonRequest
            {
                ...
                ThirdName = string.IsNullOrWhiteSpace(ThirdName)
                    ? null
                    : ThirdName.Trim(),
                ...
                Gender = gender,
                NationalityCountryID = SelectedCountry!.CountryId,
                ImagePath = string.IsNullOrWhiteSpace(ImagePath)
                    ? null
                    : ImagePath.Trim()
            };

            var result = await _peopleApiClient.UpdateAsync(
                PersonId,
                request);

            if (result.IsFailure)
            {
                _notifications.ShowFailure(result, "Update Person Failed");
        ...
        var addResult = await _peopleApiClient.CreateAsync(createRequest);

        if (addResult.IsFailure)
        {
            _notifications.ShowFailure(addResult, "Add Person Failed");
            SaveCompleted?.Invoke(false);
            return;
        }
    ...
    private bool ValidateInput()
    {
        if (string.IsNullOrWhiteSpace(FirstName))
            return ShowValidationMessage("First name is required.");

        if (string.IsNullOrWhiteSpace(SecondName))
            return ShowValidationMessage("Second name is required.");

        if (string.IsNullOrWhiteSpace(LastName))
            return ShowValidationMessage("Last name is required.");

        if (string.IsNullOrWhiteSpace(NationalNo))
            return ShowValidationMessage("National number is required.");

        if (string.IsNullOrWhiteSpace(Phone))
            return ShowValidationMessage("Phone number is required.");

        if (SelectedCountry is null)
            return ShowValidationMessage("Nationality country is required.");
    ...
    {
        _userNotifications.ShowWarning(message, "Validation Error");
        return false;
    }
