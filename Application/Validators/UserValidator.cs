...
{
    private static readonly string[] ReservedUsernames = ["admin", "root", "system", "null"];

    // CREATE
    public static Result ValidateCreateUser(CreateUserDto dto)
    {
        if (dto is null)
    ...
    // UPDATE
    public static Result ValidateUpdateUser(UpdateUserDto dto)
    {
        if (dto is null)
            return Result.Failure("User data is required.");
    ...
    // LOGIN
    public static Result ValidateLogin(LoginRequestDto dto)
    {
        if (dto is null)
    ...
    // CHANGE PASSWORD
    public static Result ValidateChangePassword(ChangePasswordDto dto)
    {
        if (dto is null)
        ...
        if (!string.IsNullOrWhiteSpace(dto.CurrentPassword) && dto.CurrentPassword == dto.NewPassword)
    ...
    // USERNAME FORMAT
    public static Result ValidateUsernameFormat(string? username)
    {
        var errors = new List<string>();
    ...
    // RESULT
    private static Result CreateValidationResult(List<string> errors)
    {
