...
public static class DetainedLicenseValidator
{
        ...
        UpdateDetainedLicenseDto? dto)
    {
        ...
        if (dto.DetainID <= 0)
            errors.Add(
                "A valid detained license ID is required.");
        ...
        if (dto.IsReleased &&
            dto.ReleaseDate is null)
        {
            errors.Add(
                "Release date is required when the license is released.");
        }

        if (!dto.IsReleased &&
            dto.ReleaseDate is not null)
        {
            errors.Add(
                "Release date cannot be provided for a non-released license.");
        }
        ...
        if (dto.IsReleased &&
            (!dto.ReleasedByUserID.HasValue ||
             dto.ReleasedByUserID.Value <= 0))
        {
            errors.Add(
                "A valid releasing user is required when the license is released.");
        }
        ...
        {
            errors.Add(
                "A valid release application is required when the license is released.");
        }
        ...
        ReleaseDetainedLicenseDto? dto)
    {
        ...
        if (dto.ReleasedByUserID <= 0)
            errors.Add(
                "A valid releasing user is required.");

        if (dto.ReleaseApplicationID <= 0)
            errors.Add(
                "A valid release application is required.");
    ...
    public static Result ValidateLicenseId(
        int licenseId)
    {
        return licenseId > 0
            ? Result.Success()
