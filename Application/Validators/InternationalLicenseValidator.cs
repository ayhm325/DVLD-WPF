...
{
    // CREATE
    public static Result ValidateCreate(CreateInternationalLicenseDto? dto)
    {
        if (dto is null)
        ...
        if (dto.IssuedUsingLocalLicenseID <= 0)
            errors.Add("A valid local license is required.");
        ...
        if (dto.IssueDate != default && dto.ExpirationDate != default && dto.ExpirationDate <= dto.IssueDate)
    ...
    // UPDATE
    public static Result ValidateUpdate(UpdateInternationalLicenseDto? dto)
    {
        if (dto is null)
            return Result.Failure("International license data is required.");
        ...
        if (dto.InternationalLicenseID <= 0)
            errors.Add("A valid international license ID is required.");
        if (dto.ApplicationID <= 0)
            errors.Add("A valid application is required.");
        if (dto.DriverID <= 0)
            errors.Add("A valid driver is required.");
    ...
    public static Result ValidateId(int id)
    {
        return id > 0
            ? Result.Success()
            : Result.Failure("Invalid international license ID.");
    }
    ...
    public static Result ValidateDriverId(int driverId)
    {
        return driverId > 0
            ? Result.Success()
            : Result.Failure("Invalid driver ID.");
    }
    ...
    public static Result ValidateLocalLicenseId(int localLicenseId)
    {
        return localLicenseId > 0
            ? Result.Success()
            : Result.Failure("Invalid local license ID.");
    }
