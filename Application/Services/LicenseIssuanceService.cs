...
public class LicenseIssuanceService : ILicenseIssuanceService
{
    ...
    public async Task<Result<int>> IssueFirstLicenseAsync(int localAppId, string? notes)
    {
        ...
        if (localApp.ApplicationStatus != AppStatus.New)
            return Result<int>.FromConflict("License can only be issued for a new application.");

        if (!await _testWorkflowService.HasPassedAllTestsAsync(localAppId))
            return Result<int>.FromConflict("The applicant has not passed all required tests.");

        if (localApp.HasLicense)
            return Result<int>.FromConflict("A license has already been issued for this application.");
        ...
        if (appValidation.IsFailure)
            return Result<int>.FromValidationFailure(appValidation.Error);
        ...
        try
        {
            ...
            else
            {
                ...
                var createDriverResult = await _driverService.AddAsync(
                    new CreateDriverDto { PersonID = personResult.Value.PersonId });
            ...
            var completeResult = await _applicationService.CompleteApplicationAsync(applicationIdResult.Value);
