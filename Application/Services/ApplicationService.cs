    ...
    ICurrentUserService currentUserService) : IApplicationService
{
    ...
    public async Task<Result<ApplicationBasicInfoDto>> GetBasicInfoAsync(int id)
    {
        ...
        {
            return Result<ApplicationBasicInfoDto>.FromValidationFailure(
                validation.Error);
        }
        ...
        return entity is null
            ? Result<ApplicationBasicInfoDto>.FromNotFound(
                "Application not found.")
            : Result<ApplicationBasicInfoDto>.Success(
    ...
    public async Task<Result<ApplicationDto>> GetApplicationByIdAsync(int id)
    {
        ...
        if (validation.IsFailure)
        {
            return Result<ApplicationDto>.FromValidationFailure(
                validation.Error);
        }
        ...
        CreateApplicationDto dto)
    {
        ...
        var entity = ApplicationMapper.ToEntity(
            dto,
        ...
        UpdateApplicationDto dto)
    {
        ...
        var entity =
            await _repository.GetApplicationForUpdateAsync(
                dto.ApplicationID);
    ...
    public async Task<Result> DeleteApplicationAsync(int id)
    {
        ...
        var entity =
            await _repository.GetApplicationForUpdateAsync(id);
    ...
    {
        var entityResult =
            await GetTrackedApplicationAsync(applicationId);
    ...
    private async Task<Result<ApplicationD>> GetTrackedApplicationAsync(
        int id)
    {
        var validation = ApplicationValidator.ValidateId(id);
        ...
        if (validation.IsFailure)
        {
            return Result<ApplicationD>.FromValidationFailure(
                validation.Error);
        }
        ...
        return entity is null
            ? Result<ApplicationD>.FromNotFound(
                "Application not found.")
